"""Codec versionado do contêiner de backup DFCRMBK1.

O codec trata o arquivo como entrada não confiável: valida cabeçalho e tamanho
físico antes do KDF, autentica cada quadro e publica saída somente ao concluir.
"""
...
class BackupContainerError(Exception):
    """Base sanitizada para um contêiner de backup que não pode ser lido."""


class InvalidBackupFormatError(BackupContainerError):
    """O arquivo não respeita o formato v1 ou seus limites seguros."""


class BackupPasswordOrIntegrityError(BackupContainerError):
    """Senha incorreta e corrupção autenticada compartilham o mesmo erro."""
...
def decrypt_payload(
    *, source_path: Path, destination_path: Path, passphrase: str
    ...
    """Valida e decifra por streaming sem deixar conteúdo parcial publicado."""
    with source_path.open("rb") as source:
        header = _read_header_from(
            source, source_size=os.fstat(source.fileno()).st_size
        )
...
def _read_header_from(source: BinaryIO, *, source_size: int) -> BackupHeader:
    ...
    expected_size = (
        ...
        + header.frame_count * (_FRAME_LENGTH.size + TAG_BYTES)
    )
    if expected_size != source_size:
        raise InvalidBackupFormatError("backup size does not match its header")

    # Verify every frame length before scrypt. The fixed frame sizes let us
    # skip ciphertext without reading it into memory during this pass.
    remaining = header.payload_bytes
    for _ in range(header.frame_count):
        ...
        if len(encoded_length) != _FRAME_LENGTH.size:
            raise InvalidBackupFormatError("backup frame is truncated")
        (plaintext_length,) = _FRAME_LENGTH.unpack(encoded_length)
        expected_length = min(FRAME_SIZE_BYTES, remaining)
        if plaintext_length != expected_length:
            raise InvalidBackupFormatError("backup frame length is invalid")
        source.seek(plaintext_length + TAG_BYTES, os.SEEK_CUR)
        remaining -= plaintext_length
    if remaining != 0 or source.tell() != source_size:
        raise InvalidBackupFormatError("backup contains missing or extra frames")
    source.seek(_PREFIX.size + header_length)
    return header
